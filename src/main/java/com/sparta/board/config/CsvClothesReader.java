...
public class CsvClothesReader {
    public ItemReader<? extends ClothesDto> csvClothesReader(){

        // 파일읽기
        FlatFileItemReader<ClothesDto> flatFileItemReader = new FlatFileItemReader<>();
        flatFileItemReader.setResource(new ClassPathResource("/서울특별시 동작구 의류수거함 현황.csv")); //읽을 파일 경로 지정
        flatFileItemReader.setLinesToSkip(1);
        flatFileItemReader.setEncoding("UTF-8"); //인코딩 설정

        // defaultLineMapper: 읽으려는 데이터 LineMapper을 통해 Dto로 매핑
        DefaultLineMapper<ClothesDto> defaultLineMapper = new DefaultLineMapper<>();
        ...
        BeanWrapperFieldSetMapper<ClothesDto> beanWrapperFieldSetMapper = new BeanWrapperFieldSetMapper<>();
        beanWrapperFieldSetMapper.setTargetType(ClothesDto.class);
